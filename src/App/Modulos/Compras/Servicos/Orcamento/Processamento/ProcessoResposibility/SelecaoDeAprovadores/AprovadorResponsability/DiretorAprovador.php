...
class DiretorAprovador extends AprovadorAbstract
{
    ...
    public function processarOrcamento(Orcamento $orcamento): void
    {
        ...
        if ($orcamento->obterValorTotal() > NivelAprovacao::PRESIDENTE->value
            || ($orcamento->getAprovadores()->count() > 2)) {
            if (! $this->sucessor) {
                return;
            }
        ...
        $funcionario = $this->funcionarioServico->obterPorFuncaoIdEDepartamentoId(
            FuncoesLideranca::DIRETOR->value,
            $departamentoId,
        );
        ...
        if ($orcamento->obterValorTotal() > NivelAprovacao::DIRETOR->value) {
